...
const Messenger = ({ socket, username, roomID }: IMessenger) => {
    ...
    useEffect(() => {
        socket.on("receive_message", (data: T[]) => {
                                ...
                                messages.map((_message: any) => (
                                        <div key={_message.key} className={`flex my-3 mx-3 ${_message.author === username ? "justify-left" : "justify-end"}`}>
                                            <div>
                                                ...
                                                <div className="text-left p-3 text-gray-500 flex flex-row space-x-12">
                                                    <p className="font-bold">{_message.time}</p>
                                                    <p className="pl-3 font-bold">{ _message.author === username ? "You:" : _message.author+": "}</p>
                                                    <p className="pl-3">{_message.message}</p>
                                                </div>
                                            </div>
                                        </div>
                                ))
